...
function getArtistByIndex(artArray, indexNumId) 
{
  return `the artist at index ${indexNumId} is ${artArray[indexNumId]["name"]}`
}  
...
function get20s(artArray)
{
  ...
  {
    let splitString = artArray[i]["years"].split("-");
if  ((splitString[0] >= 1900 && splitString[0] <= 1999) && (splitString[1] >= 1900 && splitString[1] <= 1999))
{
  newArtist.push(artArray[i]["name"]);
}
  }
  return newArtist;
}

// WOW glad I studied the .split method~! thanks for the tip :D
...
function removeArtist(artArray, indexNumId)
{
   artArray.splice(indexNumId, 1);
...
function addArtist(artArray){
  artArray.push(
    {
    id: 20,
    name: "Catherine Radin", 
    years: "1991 - 2020",
    genre: "Web Design", 
    nationality: "American",
    bio: "Born and raised in CA. She was always interested in web design. Begining at an early age she received her first computer and started designing small pages on a popular website called Neopets" 
  }
  )
  return artArray;
}
